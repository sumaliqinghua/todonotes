...
## 变更历史
- 2026-06-10：修正 Codex 路径弹窗触发条件：只有当前子页没有 `codexSessionId`、即将发起首次会话时才弹“配置项目路径”；当前子页已有 `codexSessionId`，包括手动绑定的会话 ID，后续追问直接继续已有会话，不再弹路径。首次会话在 `Codex App` 模式下允许项目路径为空，空路径时使用不带 `path` 参数的 `codex://threads/new?prompt=<提示词>`。
- 2026-06-10：恢复首次“用当前块追问 Codex”的项目路径弹窗：首次会话会先弹出“配置项目路径”；如果是 `Terminal` 模式，用户取消或输入空路径时不发起 Codex 对话。
- 2026-06-10：新增手动绑定/换绑本页 Codex 会话 ID：当前子页右键菜单提供“设置本页 Codex 会话 ID”，用户可以填入已有 Codex 会话 ID 并保存到 `codexSessionId`；后续“用当前块追问 Codex”会把该 ID 当作已有会话使用，Terminal 模式走 `codex exec resume <SESSION_ID> <PROMPT>`，Codex App 模式打开 `codex://threads/<SESSION_ID>` 并复制带回调信息的 prompt；如需解除绑定，继续使用“清除本页 Codex 会话”。
...
- Codex 子页会话第一版：
  ...
  - 子页右键文本块选择“用当前块追问 Codex”时，先判断当前页是否已有 `codexSessionId`。
  - 如果当前页已有 `codexSessionId`，包括用户通过“设置本页 Codex 会话 ID”手动绑定的 ID，本次追问直接继续该会话，不弹“配置项目路径”。
  - 如果当前页没有 `codexSessionId`，表示即将创建首次会话，此时才弹出“配置项目路径”输入框。
  - 首次会话在 `Terminal` 模式下必须填写项目路径；用户取消或提交空路径时，本次追问取消。
  - 首次会话在 `Codex App` 模式下允许项目路径为空；用户提交空路径时，会打开不带 `path` 参数的 `codex://threads/new?prompt=<URL_ENCODED_PROMPT>`，作为纯对话发起。
  ...
  - 右键菜单提供“设置本页 Codex 会话 ID”：用户可以把当前子页手动绑定到一个已有 Codex 会话，也可以把已有绑定改成另一个会话 ID。
  - 手动保存会话 ID 时会写入当前任务页的 `codexSessionId`；输入框默认显示当前值，便于直接编辑换绑。
  - 空会话 ID 不会写入；如果用户要解除绑定，应使用“清除本页 Codex 会话”。
  - 当前子页没有 `codexSessionId` 时，首次追问通过 `codex exec --json --cd <DIR> <prompt>` 创建新会话，并从 JSONL 事件里的 `thread_id` 保存会话 ID。
  - 当前子页已有 `codexSessionId` 时，无论该 ID 来自 Codex CLI 自动写回、Codex App 回调，还是用户手动设置，后续文本块都会通过 `codex exec resume --json <SESSION_ID> <prompt>` 继续同一对话。
  ...
  - 右键菜单提供“清除本页 Codex 会话”；确认后只清空当前子页的 `codexSessionId`，保留 `codexCwd`，下次追问按新会话发起。
