...
- Codex 子页会话第一版：
  ...
  - AI 处理中复用块状态 `waiting`，等待原因写为 `AI处理中`；AI 成功结束后转为 `doing`；AI 失败后保持 `waiting`，等待原因写为 `失败`。
  - 右键菜单提供“打开本页 Codex 会话”；没有 `codexSessionId` 时禁用；有会话时优先尝试官方 `codex://threads/<SESSION_ID>` 跳转 Codex App，失败时在 macOS Terminal 中执行 `codex resume <SESSION_ID>`。
